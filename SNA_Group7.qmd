...
|----|:---|
| **H** $_
1^a$ | Borrower networks based on similarity have a low degree of centralisation |
...
| Hypothesis | T ype | D yad | ERGM Term | Motivation |
|----|----|----|----|----|
| **H** $^a_2$: Younger borrowers tend to form more r elationships with each other | E xo. | Ind ep. | `n odecov()` | The `nodecov()` term captures how the age covariate affects the likelihood of edge formation. Since we want a numeric range, this term is most appropriate. |
| **H** $^a_3$: Women tend to form more r elationships with other borrowers | E xo. | Ind ep. | `node factor()` | The `nodefactor()` term captures the tendency of women to form more edges, regardless of the other gender. |
| **H** $^a_4$: Borrowers tend to exhibit herding around each other | En do. | Int er. | `gwesp()` | The `gwesp()` term captures the tendency for triadic relationships to form. In other words, for ties to form due to the presence of other ties in the dyad's neighbourhood. |
| **H** $^a_5$: Borrowers tend to form ties around other hi gh-status borrowers | En do. | Int er. | `gwd()` and `n odecov()` | The `gwd()` term describe degree distribution, indicating whether a borrower is popular to the degree specified. It does not necessarily relate to community. It is necessary to also use an indicator of prestige to understand whether pipe or prism dynamics may be more prevalent. |
